...
- Name: Authentication API >> should not register user with invalid email
- Location: /home/ramonkey/Developer/fs1-mern/back/tests/api/auth.test.js:44:3
...
Expected: 400
Received: 409
    at /home/ramonkey/Developer/fs1-mern/back/tests/api/auth.test.js:52:31
```
...
```ts
   ...
   2 | const checkServerHealth = require('../utils/healthCheck');
   3 |
   4 | test.describe('Authentication API', () => {
   5 |   let authToken;
   6 |   let adminToken;
   7 |   const testUser = {
   8 |     username: `testuser_${Date.now()}`,
   9 |     email: `test_${Date.now()}@test.com`,
   ...
   14 |
   15 |   const adminUser = {
   16 |     email: 'admin@elmundodelasguitarras.com',
   17 |     password: 'admin123' // You may need to update this with the actual admin password
   18 |   };
   19 |
   20 |   // Add health check before running auth tests
